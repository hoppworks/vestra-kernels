...
/// Experimental BLIS-backed DA3 attention.
///
/// The established flash path is normally superior because it avoids the
/// `tokens × tokens` score matrix. On the fixed 865-token CPU workload,
/// however, two large BLIS SGEMMs may amortize scheduling and SIMD overhead
/// more effectively than the many small hand-written tiles. The route is
/// strictly opt-in and allocates its score matrix once per invocation; it
/// must pass the full C++ F32 gate before it can be considered for admission.
#[cfg(da3_blis)]
pub fn blis_attention_f32_da3_base(
    q: &[f32],
    k: &[f32],
    v: &[f32],
    heads: usize,
    ...
    if std::env::var_os("DA3_KERNELS_BLIS_FLASH").is_none()
        || tokens != DA3_BASE_TOKENS_504X336
        || q.len() != heads * tokens * 64
        || k.len() != q.len()
        || v.len() != q.len()
        || out.len() != q.len()
    ...
    let mut packed_k = vec![0.0f32; 64 * tokens];
    let mut scores = vec![0.0f32; tokens * tokens];
    for head in 0..heads {
        let base = head * tokens * 64;
        let q_head = &q[base..base + tokens * 64];
        let k_head = &k[base..base + tokens * 64];
        let v_head = &v[base..base + tokens * 64];
        let out_head = &mut out[base..base + tokens * 64];

        // BLIS expects B in row-major `[K, N]`; transposing K once gives the
        // exact Q × K^T operand without asking BLIS for a runtime transpose.
        for key in 0..tokens {
            for dim in 0..64 {
                packed_k[dim * tokens + key] = k_head[key * 64 + dim];
            }
        }
        if !blis_sgemm_row_major(tokens, tokens, 64, q_head, &packed_k, &mut scores) {
            return false;
        }

        scores.par_chunks_mut(tokens).for_each(|row| {
            ...
            if std::is_x86_feature_detected!("avx512f") {
                // SAFETY: feature detection above gates the shared SIMD
                // exponential used by the accepted flash implementation.
                unsafe { crate::simd_avx512::exp_in_place_avx512(row) };
            } else {
                for value in row.iter_mut() {
                    *value = value.exp();
                }
        ...
        if !blis_sgemm_row_major(tokens, 64, tokens, &scores, v_head, out_head) {
            return false;
        }
    }
    true
