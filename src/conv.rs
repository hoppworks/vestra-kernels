...
/// Immutable composition of a stride-4 4×4 transposed convolution followed
/// immediately by a bias-free 3×3, pad-1 convolution.
///
/// This is the exact linear pair used by DA3 BASE's first DPT reassemble
/// stage.  The intermediate map is intentionally absent from this format:
/// the composed 6×6 IO kernel writes the final lateral channels directly.
/// The resize bias cannot be folded into one output-channel bias because the
/// following padded convolution sees fewer neighbours at the image edges;
/// [`conv_transpose_lateral_composed`] therefore applies it with the original
/// boundary rule.
#[derive(Clone)]
pub struct ComposedTransposeLateralF32 {
...
/// Prepares the linear composition described by [`ComposedTransposeLateralF32`].
///
/// `transpose_weight` uses PyTorch/GGUF IOHW layout, while
/// `lateral_weight` uses OIHW layout. The fixed 4×4 / 3×3 geometry is
/// deliberately explicit: this is a narrow DA3 kernel, not a second generic
/// convolution framework.
pub fn prepare_composed_transpose_lateral_filter(
    transpose_weight: &[f32],
    transpose_bias: &[f32],
    lateral_weight: &[f32],
    input_channels: usize,
    middle_channels: usize,
    ...
    assert_eq!(
        transpose_weight.len(),
        input_channels * middle_channels * 4 * 4,
        "expected IOHW 4x4 transpose weights"
    );
    assert_eq!(
        transpose_bias.len(),
        middle_channels,
        "transpose bias shape"
    );
    assert_eq!(
        lateral_weight.len(),
        output_channels * middle_channels * 3 * 3,
        "expected OIHW 3x3 lateral weights"
    );

    let mut weights = vec![0.0; output_channels * input_channels * 6 * 6];
    for output in 0..output_channels {
        for input in 0..input_channels {
            for transpose_y in 0..4 {
                for transpose_x in 0..4 {
                    for lateral_y in 0..3 {
                        for lateral_x in 0..3 {
                            // y = 4 * input_y + transpose_y - (lateral_y - 1)
                            // so its displacement is in [-1, 4].
                            let composed_y = transpose_y + 2 - lateral_y;
                            let composed_x = transpose_x + 2 - lateral_x;
                            let composed_index =
                                (((output * input_channels + input) * 6 + composed_y) * 6)
                                    + composed_x;
                            for middle in 0..middle_channels {
                                let transpose_index =
                                    ((input * middle_channels + middle) * 4 + transpose_y) * 4
                                        + transpose_x;
                                let lateral_index =
                                    ((output * middle_channels + middle) * 3 + lateral_y) * 3
                                        + lateral_x;
                                weights[composed_index] += transpose_weight[transpose_index]
                                    * lateral_weight[lateral_index];
                            }
                        }
                    }
                }
...
/// Executes a prepared DA3 stage-0 transpose/lateral composition.
///
/// This deliberately uses output-owned gathers: each output element is
/// initialized exactly once, avoiding the races and temporary buffer required
/// by a parallel overlapping transposed-convolution scatter. The current
/// scalar reference route exists to prove the composition and edge bias
/// contract; an AVX-512 polyphase implementation can replace its inner loop
/// without changing this public representation.
pub fn conv_transpose_lateral_composed(
    input: &[f32],
    ih: usize,
    iw: usize,
    ...
    assert_eq!(input.len(), filter.input_channels * ih * iw);
    let oh = ih * 4;
    let ow = iw * 4;
    ...
    out.par_chunks_mut(oh * ow)
        .enumerate()
        .for_each(|(output, plane)| {
            for oy in 0..oh {
                for ox in 0..ow {
                    let mut acc = 0.0f32;
                    for input_channel in 0..filter.input_channels {
                        for kernel_y in 0..6 {
                            let displacement_y = kernel_y as isize - 1;
                            let numerator_y = oy as isize - displacement_y;
                            ...
                            for kernel_x in 0..6 {
                                let displacement_x = kernel_x as isize - 1;
                                let numerator_x = ox as isize - displacement_x;
                                ...
                                let weight_index =
                                    (((output * filter.input_channels + input_channel) * 6
                                        + kernel_y)
                                        * 6)
                                        + kernel_x;
                                let input_index = (input_channel * ih + source_y) * iw + source_x;
                                acc += input[input_index] * filter.weights[weight_index];
                            }
                        }
                    }

                    // The intermediate transpose bias is present at every
                    // in-bounds point of its 4x upsampled rectangle. The
                    // 3×3 lateral sees it through exactly the same padded
                    // neighbourhood as the materialized reference route.
                    for middle in 0..filter.middle_channels {
                        let bias = filter.transpose_bias[middle];
                        for lateral_y in 0..3 {
                            let source_y = oy as isize + lateral_y as isize - 1;
                            if source_y < 0 || source_y >= oh as isize {
                                continue;
                            }
                            for lateral_x in 0..3 {
                                let source_x = ox as isize + lateral_x as isize - 1;
                                if source_x < 0 || source_x >= ow as isize {
                                    continue;
                                }
                                let lateral_index =
                                    ((output * filter.middle_channels + middle) * 3 + lateral_y)
                                        * 3
                                        + lateral_x;
                                acc += bias * filter.lateral_weights[lateral_index];
                            }
                        }
                    }
                    plane[oy * ow + ox] = acc;
...
mod tests {
    ...
    #[test]
    fn composed_stage0_transpose_lateral_matches_materialized_signed_rectangles() {
        // Exercise corners, non-square inputs and a normal interior. The
        ...
        let lateral_weight = random_vec(&mut rng, out_c * middle_c * 3 * 3);
        let filter = prepare_composed_transpose_lateral_filter(
            &transpose_weight,
            &transpose_bias,
            &lateral_weight,
            in_c,
        ...
        for &(ih, iw) in &[(1usize, 1usize), (1, 3), (2, 1), (2, 3), (5, 4)] {
            let input = random_vec(&mut rng, in_c * ih * iw);
            let (oh, ow) = (ih * 4, iw * 4);
            let mut materialized = vec![0.0; middle_c * oh * ow];
            conv_transpose2d(
                &input,
                in_c,
                ih,
            ...
            conv_transpose_lateral_composed(&input, ih, iw, &filter, &mut actual);
            ...
            assert!(
                max_abs <= 2e-5,
                "shape={ih}x{iw} max_abs={max_abs}; composition or edge bias diverged"
            );
    ...
    #[test]
    fn composed_stage0_transpose_lateral_retains_edge_dependent_bias() {
        // Zero activations isolate the often-missed spatial bias correction:
        // the centre sees all nine lateral taps, corners only four.
        let (in_c, middle_c, out_c, ih, iw) = (1usize, 2usize, 1usize, 2usize, 3usize);
        let transpose_weight = vec![0.0; in_c * middle_c * 4 * 4];
        let transpose_bias = vec![0.5, -0.75];
        let lateral_weight = vec![
            0.1, -0.2, 0.3, // middle 0
            -0.4, 0.5, -0.6, 0.7, -0.8, 0.9, -0.15, 0.25, -0.35, // middle 1
        ...
        let max_abs = expected
            ...
            .map(|(expected, actual)| (expected - actual).abs())
            .fold(0.0f32, f32::max);
        assert!(max_abs <= 1e-6, "edge bias max_abs={max_abs}");
        assert_ne!(actual[0], actual[ow + 1], "corner must not use centre bias");
    }
