...
unsafe fn flash_attention_tile_packed_8x32_avx512(
    ...
    for k0 in (0..tokens).step_by(KVT) {
        ...
        let v_gemm_started = profile.enabled.then(Instant::now);
        let score_flat = unsafe { core::slice::from_raw_parts(scores.as_ptr().cast::<f32>(), 8 * KVT) };
        let accum_flat = unsafe { core::slice::from_raw_parts_mut(accum.as_mut_ptr().cast::<f32>(), 8 * D) };
        unsafe {
            if cols == KVT {
...
unsafe fn flash_attention_avx512(
    ...
    let packed_qt8 = gemm_8x32
        && std::env::var_os("DA3_KERNELS_DISABLE_FLASH_PACKED_QT8").is_none();
